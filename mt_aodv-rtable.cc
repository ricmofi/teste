...
NS_LOG_COMPONENT_DEFINE ("Mt_aodvRoutingTable");
...
/*
  int x;
  std::cin >> x;
  std::cout<< "linha 63 mt_aodv-rtable.cc";
*/

  m_ipv4Route->SetDestination (dst);
  //m_ipv4Route->SetGatewaymt_aodv (std::make_pair(nextHop,hops)); //03MAR
  //m_ipv4Route->SetNextHopList({std::make_pair(nextHop,hops)}); //03MAR
  m_ipv4Route->SetGateway (nextHop); 
 
  m_ipv4Route->SetSource (m_iface.GetLocal ());
  m_ipv4Route->SetOutputDevice (dev);

   //std::cout<<"CONSTRUTOR - RoutingTableEntry::RoutingTableEntry "<< "-dst: " << dst<< " seqNo: "<< seqNo<<
//" iface "<< iface<<" hops: "<<hops<< " nextHop "<< nextHop<< " lifetime "<< lifetime<<std::endl;
}
...
{  //std::cout<<"RoutingTableEntry::InsertPrecursor Ipv4Address id " << id<<std::endl;
...
{ //std::cout<<"RoutingTableEntry::LookupPrecursor Ipv4Address id " << id<<std::endl;
  NS_LOG_FUNCTION (this << id);
...
{ //std::cout<<"RoutingTableEntry::DeletePrecursor Ipv4Address id " << id<<std::endl;
...
{
  //std::cout<<"RoutingTableEntry::DeleteAllPrecursors"<<std::endl;
...
{
  //std::cout<<"RoutingTableEntry::IsPrecursorListEmpty"<<std::endl;
...
{
  //std::cout<<"RoutingTableEntry::GetPrecursors (std::vector<Ipv4Address> & prec) const"<<std::endl;
...
{
  //std::cout<<"RoutingTableEntry::Invalidate (Time badLinkLifetime)"<<std::endl;
...
{
  //std::cout<<"RoutingTableEntry::Print "<<std::endl;

  std::ostream* os = stream->GetStream ();
  //*os << m_ipv4Route->GetDestination () << "\t" << m_ipv4Route->GetGatewaymt_aodv ().first
...
RoutingTable::RoutingTable (Time t)
  : m_badLinkLifetime (t)
{
  //std::cout<<"RoutingTable::RoutingTable "<<std::endl;
...
{
  //std::cout<<"RoutingTable::LookupRoute (Ipv4Address id, RoutingTableEntry & rt)" <<" id "<< id <<std::endl;
...
bool
RoutingTable::LookupGateway (Ipv4Address id, Ipv4Address gtw, RoutingTableEntry & rt)
{
  NS_LOG_FUNCTION (this << id);
  Purge();
 if (m_ipv4AddressEntry.empty ())
...
{
  //std::cout<<"RoutingTable::LookupValidRoute (Ipv4Address id, RoutingTableEntry & rt)" <<" id "<< id <<std::endl;
...
{
  //std::cout<<"RoutingTable::DeleteRoute (Ipv4Address dst)" <<" dst "<< dst <<std::endl;
...
{
  //std::cout<<"RoutingTable::AddRoute (RoutingTableEntry & rt)" <<std::endl;
  ...
  if (rt.GetFlag () != IN_SEARCH)
    ...
    //std::cout<< "Teste2 - AddRoute"<< rt.GetDestination ()<< "gateway" << rt.GetNextHop()<<std::endl;
...
{ //std::cout<<"RoutingTable::Update " << " Destination: " <<  rt.GetDestination() << " Hop " << rt.GetHop() << " NextHop "<<rt.GetNextHop()<<std::endl;
  NS_LOG_FUNCTION (this);
...
{ //std::cout<<"RoutingTable::SetEntryState (Ipv4Address id, RouteFlags state)" << " id "<<id<<" state "<<state<<std::endl;
...
{
 //std::cout<<"RoutingTable::GetListOfDestinationWithNextHop (Ipv4Address nextHop, std::map<Ipv4Address, uint32_t> & unreachable )"
 //<< " nextHop "<< nextHop <<std::endl;
...
{
 //std::cout<<"RoutingTable::InvalidateRoutesWithDst (const std::map<Ipv4Address, uint32_t> & unreachable)"<<std::endl;
...
{
 //std::cout<<"RoutingTable::DeleteAllRoutesFromInterface (Ipv4InterfaceAddress iface) "<< iface<<std::endl;
...
RoutingTable::Purge ()
{
 //std::cout<<"RoutingTable::Purge() "<< std::endl;
...
{
 //std::cout<<"RoutingTable::Purge(std::map<Ipv4Address, RoutingTableEntry> &table) const "<<std::endl;
...
{
  //std::cout<<"RoutingTable::MarkLinkAsUnidirectional (Ipv4Address neighbor, Time blacklistTimeout)"
